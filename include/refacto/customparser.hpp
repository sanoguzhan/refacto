...
struct Entity
{
  ...
  Entity(string key, string name, string orient, string eType, string keyword, int column, int vFrom, int vTo)
    : key{ key }, name{ name }, orient{ orient }, eType{ eType }, keyword{ keyword }, column{ column }, vFrom{ vFrom },
      vTo{ vTo }
  {}
  ...
  Entity(string key,
    ...
    : key{ key }, name{ name }, orient{ orient }, eType{ eType }, row{ row }, column{ column },
      value_begin{ value_begin }, keyword{ keyword }
  {}

  // Series initilization
  Entity(string key, string name, string orient, string eType, int row, int value_begin = 0)
    : key{ key }, name{ name }, orient{ orient }, eType{ eType }, row{ row }, value_begin{ value_begin }
  {}
  ...
  Entity(string key,
    ...
    vector<map<string, string>> conditions,
    int value_begin = 0)
    : key{ key }, name{ name }, orient{ orient }, eType{ eType }, value_begin{ value_begin }, conditions{ conditions }
  {}
...
public:
  ...
  template<typename... T> CustomParser(T... lst) : meta{ lst... } {}
...
public:
   ...
   */
  void in_insert(shared_ptr<Table> tb, const vector<Entity> &entity_lst, Series &series, vector<string> &vec_values);
   ...
   */
  void from_group_series(Series &series, const Entity &item, shared_ptr<Table> tb);
...
private:
   ...
   */
  void construct_series(Series &series, const Entity &item, shared_ptr<Table> tb);

  void inline clean_series(Series &series) const
  {
    for (auto &p : series.values) { series.values.find(p.first)->second.clear(); }
  }
   ...
   * @param tb (shared_ptr<Table>) tables for insertion
   */
  void construct_series(Series &series, const Entity &item, map<string, string> target, shared_ptr<Table> tb) const;
   ...
   * @return std::string
   */
  string get_substring(std::string delimiter, std::string extention, std::string s) const;
};

inline bool operator>(const Entity &right, const Entity &left) { return right.key > left.key; }
inline bool operator<=(const Entity &right, const Entity &left) { return right.key <= left.key; }
inline bool operator<(const Entity &right, const Entity &left) { return right.key < left.key; }
