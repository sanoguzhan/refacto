...
public:
  ...
  vector<Entity> validate(vector<map<string, string>> values, map<string, map<string, string>> condition)
  {
    vector<Entity> entity;

    for (auto i : values) {
      map<string, string>::iterator condition_name{ i.find("condition") };
      if (i.at("type") == "series" || i.at("type") == "group") {
  ...
  void save(string dir) { parser.to_csv(dir); }

  void operator()(string dir_path, string delimeter, int skip) { parser(dir_path, delimeter, skip); }

  void init_series_entity_pyx(map<string, string> &vals, vector<Entity> &entity)
  {
    entity.emplace_back(vals.at("key"), vals.at("name"), vals.at("type"), vals.at("keyword"));
  }

  void init_vector_ids_pyx(map<string, string> &vals, vector<Entity> &entity)
  {
  ...
  void init_series_ids_pyx(map<string, string> &vals, vector<Entity> &entity)
  {
  ...
  void init_series_pyx(map<string, string> &vals, vector<Entity> &entity, map<string, string> &condition)
  {
    vector<map<string, string>> cond;
    cond.push_back(
      { { "name", condition.at("name") }, { "orient", condition.at("orient") }, { "row", condition.at("row") } });
  ...
  void init_series_group_pyx(map<string, string> &vals, vector<Entity> &entity, map<string, string> &condition)
  {
    vector<map<string, string>> cond;


    cond.push_back({ { "id", condition.at("id") }, { "name", condition.at("name") } });
  ...
  {
    if (not_exists(vals, "key") || not_exists(vals, "name") || not_exists(vals, "orient") || not_exists(vals, "type")
        || not_exists(vals, "row") || not_exists(vals, "value_begin"))
...
class CSVParserWrapper
{
   ...
   *  Table csvparser run this wrapper together to aviod returning any values
   * to Python
   */
public:
  ...
  CSVParserWrapper(dict kwargs) : table{ make_shared<Table>() }
  {
    if (kwargs.find("csvparser") != kwargs.end()) { init_csvparser(kwargs.at("csvparser")); }
  }
  ...
  void inline from_csv2(string orient, u_int32_t idx, const Loc &target, const Loc &cond1, const Loc &cond2)
  {
    table->insert((*csvparser)(orient, idx, cond1, cond2));
  }

  void inline from_csv1(string orient, u_int32_t idx, const Loc &target, const Loc &cond1)
  {
    table->insert((*csvparser)(orient, idx, target, cond1));
  }

  void inline from_csv_vec(string column, string orient, int32_t idx, int32_t from, int32_t to)
  {
    table->insert(column, (*csvparser)(orient, idx, from, to));
  }
  void inline from_csv(string column, const Loc &target) { table->insert(column, (*csvparser)(target)); }

  void inline from_csv_static(string column, string value) { table->insert(column, value); }

  void inline to_csv(string path) { table->save(path); }
};

inline vector<IDMap> clean_values(vector<map<string, string>> kwargs, map<string, map<string, string>> condition)
{
  vector<IDMap> values;
  for (const auto &p : kwargs) {
    if (p.at("type") == "single") {
      values.emplace_back(p.at("name"), p.at("node"), p.at("key"), p.at("degree"), p.at("type"), p.at("output"));
    } else if (p.at("type") == "multi") {
      values.emplace_back(p.at("name"), p.at("node"), p.at("key"), p.at("degree"), p.at("type"));
...
//  That is why we take all parameters of IDMap as vector of maps and construct
//  in cpp if we do other way around, it makes buggy code to create this vector
//  of IDMap then we need to be careful not create python objects on the cython
//  part etc. This solution allows as to create a layer of abstraction where we
//  can also validate input
class XMLParserController : public XMLParser
{
public:
  XMLParserController(vector<map<string, string>> lst, map<string, map<string, string>> condition)
    : XMLParser{ clean_values(lst, condition) }
  {}

  void get(string dir, const string root) { XMLParser::operator()(dir, root); }
};
#endif