...
class CustomParserWrapper {
   ...
   public:
    CustomParserWrapper(vector<map<string, string>> kwargs,
            ...
            if (i.at("type") == "series" || i.at("type") == "group") {
                if (condition_name == i.end())
                    ...
                    if (i.at("type") == "series")
                        init_series_pyx(
                            i, entity,
                            condition.at(i.at("key") + i.at("name")));
                    else
                        init_series_group_pyx(
                            i, entity,
                            condition.at(i.at("key") + i.at("name")));
                }
                ...
                throw std::runtime_error(
                    "Keys not found: [series, ids, vector]");
            }
        }
        return entity;
    }
    ...
    void init_series_entity_pyx(map<string, string>& vals,
                                vector<Entity>& entity) {
        entity.emplace_back(vals.at("key"), vals.at("name"), vals.at("type"),
                            vals.at("keyword"));
    }
    ...
    void init_vector_ids_pyx(map<string, string>& vals,
                             vector<Entity>& entity) {
        entity.emplace_back(
            vals.at("key"), vals.at("name"), vals.at("orient"), vals.at("type"),
            "vector", std::stoi(vals.at("column")), std::stoi(vals.at("from")),
            std::stoi(vals.at("to")));
    }
    ...
    void init_series_ids_pyx(map<string, string>& vals,
        ...
        if (vals.at("keyword") == "file_name") {
            entity.emplace_back(vals.at("key"), vals.at("name"),
                                vals.at("type"), vals.at("keyword"));
        } else if (vals.at("keyword") == "multi") {
            entity.emplace_back(
                vals.at("key"), vals.at("name"), vals.at("orient"),
                vals.at("type"), std::stoi(vals.at("row")),
                std::stoi(vals.at("column")), std::stoi(vals.at("value_begin")),
                vals.at("keyword"));
    ...
    void check_keys_pyx(map<string, string>& vals) {
        if (not_exists(vals, "key") || not_exists(vals, "name") ||
            not_exists(vals, "orient") || not_exists(vals, "type") ||
...
class CSVParserWrapper {
   ...
   public:
    ...
    void inline init_csvparser(map<string, string>& vars) {
        csvparser.reset();
        csvparser = make_shared<CSVParser>(vars.at("path"), vars.at("delim"),
                                           stoi(vars.at("skip")));
    }
    ...
    void inline from_csv2(string orient, u_int32_t idx, const Loc& target,
                          const Loc& cond1, const Loc& cond2) {
        table->insert((*csvparser)(orient, idx, cond1, cond2));
    }

    void inline from_csv1(string orient, u_int32_t idx, const Loc& target,
                          const Loc& cond1) {
        table->insert((*csvparser)(orient, idx, target, cond1));
    }

    void inline from_csv_vec(string column, string orient, int32_t idx,
                             int32_t from, int32_t to) {
        table->insert(column, (*csvparser)(orient, idx, from, to));
    }
    void inline from_csv(string column, const Loc& target) {
