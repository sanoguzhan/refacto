...
struct DECOMPRESSDIR{
     ...
     *          - gzip files
     * 
     *  Runs system call ( linux only ) to find the files with the correct extensions
     *  Uses the tar, gzip zip operations
     * 
     */
    string extension;
     ...
     * @param extension 
     */
    DECOMPRESSDIR(string extension="") 
        :extension{extension}{}
    ...
    void operator()(string path){
        ...
        if(extension == "gz") cmd="gunzip -k {} \\;";
        else if(extension == "tar") cmd="tar -xzvf {} -C" + cwd +"/"+ path + " \\;";
        else if(extension == "zip") cmd="unzip {} -d " + cwd +"/"+ path + " \\;";
        std::ostringstream stringStream;
        stringStream << "yes 2>/dev/null" << " | " << "find " << path << " -name '*."<< extension << "' -type f -exec " << cmd ;
...
struct COMPRESSDIR{
    ...
    void operator()(string path, string name){
        ...
        stringStream << "tar -czvf " << cwd << "/"<<name << ".tar.gz "<< path;
        std::system(stringStream.str().c_str());
    }
};
...
string find_ext( string dir_path ){
   ...
   if ( dirFile != NULL ){
      ...
      while (( hFile = readdir( dirFile )) != NULL ){
         ...
         if ( strstr( hFile->d_name, GZipExtension)) return GZipExtension;
         else if ( strstr( hFile->d_name, ZipExtension)) return ZipExtension;
         else if ( strstr( hFile->d_name, TarExtension)) return TarExtension;
