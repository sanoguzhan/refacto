...
public:
  /**
   * @brief resulting data of the parsing composed of 2D array with 1th axis as
   * row and 2nd as column
   *
   */
   ...
   * @return Series
   */
  Series operator()(string orient, u_int32_t idx, const Loc &target,
   ...
   */
  std::vector<string> operator()(std::string orient, int32_t idx, int32_t from,
                                 int32_t to) const;
  /**
   ...
   * @return std::string
   */
  inline std::string operator()(const Loc &target) {
...
private:
   ...
   * @return std::string
   */
  std::string get_substring(std::string delimiter, std::string extention,
                            std::string s) const;
};
#endif