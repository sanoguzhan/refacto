...
void CustomParser::create_ids(Series &series, const Entity &item) {
    if (item.keyword == "file_name") {
        ...
        if (!id_exist(series, data.at(item.row).at(item.column))) {
            series.values.insert(std::make_pair(
                data.at(item.row).at(item.column), vector<string>()));
            id_type = "single";
        ...
        for (auto i : idx_vector) {
            if (!id_exist(series, data.at(item.value_begin).at(i))) {
                ...
                series.values.insert(std::make_pair(
                    data.at(item.value_begin).at(i), vector<string>()));
            }
        }
        id_type = "multi";
        idx_vector.clear();
...
void CustomParser::transform_keys(map<string, vector<Entity>> &keys) {
    ...
    transform(
        meta_names.begin(), meta_names.end(), std::inserter(keys, keys.end()),
    ...
    transform(meta_names.begin(), meta_names.end(),
              std::inserter(tables, tables.end()), [](string &key) {
