...
string CustomParser::get_variable_name(map<string, string> mapping) const {
  std::regex e_name(mapping.at("name"));
  std::cmatch cm_name;
  for (size_t c = 0; c < data.at(0).size(); c++) {
    std::regex_match(data.at(0).at(c).c_str(), cm_name, e_name,
...
void CustomParser::in_insert(shared_ptr<Table> tb,
                             const vector<Entity> &entity_lst, Series &series,
                             vector<string> &vec_values) {
...
void CustomParser::from_group_series(Series &series, const Entity &item,
  ...
  vector<string> rows;
  for (size_t c = 0; c < data.at(0).size(); c++) {
    std::regex_match(data.at(0).at(c).c_str(), cm_id, e_id,
                     std::regex_constants::match_default);
    if (cm_id.size() > 0) {
      ...
      if (!id_exist(series, cm_id[1])) {
        ...
        series.values.find(cm_id[1])->second.insert(
            std::end(series.values.find(cm_id[1])->second), std::begin(rows),
            std::end(rows));
      }
      rows.clear();
    }
...
void CustomParser::from_vector(vector<string> &vec_values, Entity &item) const {
  ...
  if (item.orient == "row") {
    if (item.vTo == -1)
      item.vTo = data.at(item.column).size();
    ...
    for (const auto &col : data) {
      if (counter == item.vTo)
        break;
      if (counter >= item.vFrom)
        vec_values.push_back(col.at(item.column));
      counter++;
    }
  } else {
  ...
  if (item.keyword == "file_name") {
    ...
    if (!id_exist(series, data.at(item.row).at(item.column))) {
      series.values.insert(
          std::make_pair(data.at(item.row).at(item.column), vector<string>()));
      id_type = "single";
    ...
    for (auto i : idx_vector) {
      if (!id_exist(series, data.at(item.value_begin).at(i))) {
        ...
        series.values.insert(
            std::make_pair(data.at(item.value_begin).at(i), vector<string>()));
      }
    }
    id_type = "multi";
    idx_vector.clear();
...
void CustomParser::construct_series(Series &series, const Entity &item,
                                    shared_ptr<Table> tb) {
  vector<string> rows;
  ...
  if (id_type == "single") {
    for (const u_int32_t col : data_idx) {
      ...
      series.values.find(id_iter)->second.insert(
          std::end(series.values.find(id_iter)->second), std::begin(rows),
          std::end(rows));
      rows.resize(0);
    }
    tb->insert(series);
    ...
    for (const u_int32_t col : data_idx) {
      ...
      series.values.find(*id_iterator)
          ->second.insert(std::end(series.values.find(*id_iterator)->second),
  ...
  for (const u_int32_t col : data_idx) {
    ...
    if (series.values.find(current_id) == series.values.end()) {
      ...
      series.values.find(current_id)
          ->second.insert(std::end(series.values.find(current_id)->second),
                          std::begin(rows), std::end(rows));
    }
    rows.resize(0);
    tb->insert(series);
...
void CustomParser::transform_keys(map<string, vector<Entity>> &keys) {
  ...
  std::transform(meta.begin(), meta.end(), back_inserter(meta_names),
                 [](Entity &id) { return id.key; });
  meta_names.erase(unique(meta_names.begin(), meta_names.end()),
                   meta_names.end());

  // Groups all entities by key name
  transform(meta_names.begin(), meta_names.end(),
            std::inserter(keys, keys.end()),
            [](string &key) { return std::make_pair(key, vector<Entity>()); });

  // Creates and group tables by key name
  transform(
      meta_names.begin(), meta_names.end(), std::inserter(tables, tables.end()),
      [](string &key) { return std::make_pair(key, make_shared<Table>()); });

  for_each(meta.begin(), meta.end(),
           [&](Entity &id) { keys.at(id.key).emplace_back(id); });
}

string CustomParser::get_substring(std::string delimiter, std::string extention,
                                   std::string s) const {
  std::string token;
  size_t pos = 0;
...
void find_ids(s_matrix &data, vector<u_int32_t> &indexes,
              map<string, string> cond, const Entity &item) {
