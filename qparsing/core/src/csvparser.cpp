...
CSVParser::CSVParser(string path, string delim, int skip_rows)
    : file_path{path}, f(validate_f(path)), skip_rows{skip_rows}, parser{f},
      delim{delim}, data{read(skip_rows)} {
  file_name = get_substring(NAV, ".", path);
}

CSVParser::CSVParser(string path, string delim)
    : file_path{path},
      f(validate_f(path)), parser{f}, delim{delim}, data{read(skip_rows)} {
  file_name = get_substring(NAV, ".", path);
}

CSVParser::CSVParser(string path, int skip_rows)
    : file_path{path},
      f(validate_f(path)), skip_rows{skip_rows}, parser{f}, data{read(
                                                                skip_rows)} {
  file_name = get_substring(NAV, ".", path);
}

CSVParser::CSVParser(string path)
    : file_path{path}, f(validate_f(path)), parser{f}, data{read(skip_rows)} {
  file_name = get_substring(NAV, ".", path);
}
...
void CSVParser::erase_diverge_row() {
  ...
  data.erase(std::remove_if(data.begin(), data.end(),
                            [&columns_number](auto columns) {
                              return (columns.size() != columns_number);
                            }),
             data.end());
}
...
Series CSVParser::operator()(string orient, u_int32_t idx, const Loc &target,
                             const Loc &cond1, const Loc &cond2) const {
...
Series CSVParser::operator()(string orient, const Loc &target,
                             u_int32_t idx) const {
  std::vector<string> rows;
...
std::vector<std::string> CSVParser::operator()(std::string orient, int32_t idx,
  ...
  if (orient == "row") {
    if (to == -1)
      to = data.at(idx).size();
    std::vector<string> cols(data.at(idx).begin() + from,
                             data.at(idx).begin() + to);
    return cols;
  } else if (orient == "column") {
    if (to == -1)
      to = data.size();
...
std::string CSVParser::get_substring(std::string delimiter,
...
bool CSVParser::to_csv(string path) const {
  ...
  for (const auto &row_data : data) {
    ofs << std::accumulate(row_data.begin(), row_data.end(),
