...
bool Table::save(string path) const
{
  ...
  // write headers
  csv_file << "id;";
  for (; i < data.begin()->second.size() - 1; i++) ss << data.begin()->second.at(i).first << ";";
    ...
    if (c.second.size() <= 1) {
      ...
      for (; loc < size; loc++, i = 0) {
        for (i = 0; i < c.second.size() - 1; i++) {
          if (id_col) {
            try {
              ss << c.first << DELIMETER << c.second.at(i).second.at(loc) << DELIMETER;
            } catch (const std::out_of_range) {
              ss << DELIMETER;
            }
          } else {
...
bool Table::insert(Series series)
{
  for (const auto &p : series.values) {
    if (data.find(p.first) == data.end()) {
      ...
      found->second.insert(std::end(found->second), p.second.begin(), p.second.end());
    }
  }
  return true;
}
...
bool Table::insert(string name, std::vector<string> vec)
{
  for (const auto &p : data) {
    if (!column_exist(p.first, name)) {
      ...
      found->second.insert(std::end(found->second), vec.begin(), vec.end());
    }
  }
  return true;
}
    ...
    if (!column_exist(p.first, name)) {
      ...
      if (get_size(p.first, name) <= max_size()) {
        auto found = std::find_if(data.find(p.first)->second.begin(),
          data.find(p.first)->second.end(),
          [&name](const auto &p) { return p.first == name; });
        found->second.insert(std::end(found->second), max_size() - get_size(p.first, name), insert_name);
      }
    }
  }
  return true;
...
size_t Table::max_size() const
{
  ...
  for (const auto &c : data) {
    for (const auto &item : c.second) {
      if (size < item.second.size()) { size = item.second.size(); }
    }
  }
  return size;
}
...
size_t Table::get_size(string id_name, string column_name) const
{
  ...
  for (const auto &c : data) {
    for (const auto &item : c.second) {
      if (id_name == c.first && item.first == column_name) { size = item.second.size(); }
    }
  }
  return size;
}
...
{
  return std::any_of(data.at(id).begin(), data.at(id).end(), [&](const auto &p) { return col_name == p.first; });
}
