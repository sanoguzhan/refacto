...
CSVParser::CSVParser(string path, string delim) : file_path{path}, f(validate_f(path)),
                                                  parser{f}, delim{delim}, data{read(skip_rows)}
{
    file_name = get_substring(NAV, ".", path);
}

CSVParser::CSVParser(string path, int skip_rows) : file_path{path}, f(validate_f(path)),
...
CSVParser::CSVParser(string path) : file_path{path}, f(validate_f(path)),
...
Series CSVParser::operator()(string orient,
                         u_int32_t idx,
                         const Loc &target,
                         const Loc &cond1,
                         const Loc &cond2)
...
std::vector<std::string> CSVParser::operator()(std::string orient,
                                           int32_t idx,
                                           int32_t from,
                                           int32_t to)
{
                    ...
                    const Loc &cond2)
{
    ...
    for (size_t c = 0; c < data.at(0).size(); c++)
    {
        ...
        if (lookup1.find(cond1.name) != string::npos && lookup2.find(cond2.name) != string::npos)
        {
            indexes.push_back(c);
        }
    }
...
bool CSVParser::to_csv(string path)
{
