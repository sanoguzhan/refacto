...
TEST(DtableTests, DataAttributes) {
    ...
    ASSERT_EQ(table["power_alternate_current"].size(), 0)
        << "Vectors are unequal length";
    try {
        table["date_col"];
...
TEST(DtableTests, TableMethods) {
    ...
    ASSERT_TRUE(table.insert("power_alternate_current",
                             std::vector<string>{"1", "2", "3"}));
    ASSERT_EQ(table["power_alternate_current"].size(), 3)
        << "Vectors are unequal length";
}
